...
class AccountClient(object):
    ...
    def _get_prefix(self):
        return _sha512('account'.encode('utf-8'))[0:6]

    def _get_address(self, ac_number):
        xo_prefix = self._get_prefix()
        game_address = _sha512(ac_number.encode('utf-8'))[0:64]
        return xo_prefix + game_address

    def create(self, **data):
        """ Create new account """
        return self._wrap_and_send(10, **data)

    def test(self, **data):
        print("validated data in test",data)

    def check(self, ac_number):
        """ Check account basic info """
        result = self._send_to_rest_api("state/{}".format(self._get_address(ac_number)))
        try:
    ...
    def _wrap_and_send(self, wait=None, **v_data):
